...
> 🔒 **Security note:** Covers every step including SQL functions missing from the base guide, platform-specific commands for Windows and Mac, and fixes for all common errors encountered during real installation. These files are downloaded from the official Open Brain GitHub repo by Nate B. Jones (the project creator).
...
- **Open Brain MCP Server** — a Supabase Edge Function that sits between your AI clients and your database, giving Claude and other AIs tools to read and write your memories via:

---

- `capture_thought` thoughts, decisions, facts, names, deadlines
- `search_thoughts` your memory by meaning (not just keywords)
- `list_thoughts` recent thoughts filtered by topic, person, or type
- `thought_stats` see totals, types, top topics, people mentioned

> Once connected, any AI client supporting MCP can read and write your personal memory — Claude, ChatGPT, Gemini, Cursor, VS Code. No single company owns your context.
---
...
| OpenRouter account | [openrouter.ai](https://openrouter.ai) | Free, $10 credit recommended |
| Node.js (LTS) | [nodejs.org](https://nodejs.org) | Required for Claude Code |
| Scoop (Windows only) | Installed during Step 6 | Package manager |
| Homebrew (Mac only) | [brew.sh](https://brew.sh) | Package manager |
| Credential tracker | [Download from repo](https://github.com/jon3nity/OB1/blob/main/docs/open-brain-credential-tracker.xlsx) | Keep open throughout |
...
> ⚠️ **Credential tracker:** Keep it local only. Never upload to Google Drive, OneDrive, or any cloud sync. Your Secret Key grants full database access and cannot be revoked without rotating it.
...
5. Set **Region** — choose the closest to you (West EU Ireland, US East, etc.)
6. Under **Security**:
...
Once loaded, copy your **Project Ref** from the browser URL:
```
supabase.com/dashboard/project/YOUR_PROJECT_REF
```
Paste it into credential tracker row 1.8. All AUTO rows populate immediately.
